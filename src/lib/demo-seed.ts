...
import { DEAL_STAGES, DEMO_ACCOUNTS, DEMO_PASSWORD, type DealStage } from "./constants";

// Shared by `prisma db seed` and the nightly demo reset (/api/cron/reset-demo).
// No "server-only" import: this file also runs under tsx.

const DAY = 24 * 60 * 60 * 1000;

export async function seedDemoData(prisma: PrismaClient) {
  // Deterministic pseudo-random numbers so every reset produces the same demo data.
  let state = 42;
  ...
  for (const account of DEMO_ACCOUNTS) {
    users.push(
      await prisma.user.create({
        data: { name: account.name, email: account.email, role: account.role, passwordHash },
      }),
    );
  }
  ...
  const companies = [];
  for (const [name, domain, industry, size] of companySeed) {
    companies.push(
  ...
  const notes = [
    "Budget approved for Q4. Main concern is migration effort from their current spreadsheet setup.",
    "Champion is the VP Sales; procurement wants a security questionnaire before signing.",
    "Asked for a 10% discount on a two-year term. Competitor quote is slightly lower.",
    "Trial going well: 12 active users in the first week. Wants SSO before rollout.",
  ];
  for (let i = 0; i < 40; i++) {
    ...
    await prisma.activity.create({
      data: {
        ...
        body: type === "NOTE" ? pick(notes) : null,
  ...
  return { users: users.length, companies: companies.length, contacts: contacts.length, deals: deals.length };
}
